...
int main(int argc, char** argv)
{
  ...
  MatrixXd waypoints(5, 3);
  waypoints << 8.22, -1.86, 1.6,
      8.11, -1.26, 1.7,
      7.65, -0.81, 1.62,
      7.25, -0.3, 1.57,
      7.45, 0.02, 1.33;
...
VectorXd timeAllocation( MatrixXd Path)
{
  VectorXd time = VectorXd::Zero(Path.rows() - 1);
  /*
...
The time allocation is many relative timeline but not one common timeline
  ...
  */
  for (int i = 0; i < Path.rows() - 1; i++)
  {
    ...
    time[i] = s / _Vel + _Vel / _Acc; // (t_max + t_max + 2 * _Vel / _Acc) * _Vel / 2 = s , t = t_max + 2 * _Vel / _Acc
  }
